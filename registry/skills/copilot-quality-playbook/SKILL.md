...
description: "Use for codebase quality baselines, specification-grounded audits, testing protocols, and repeatable quality systems."
license: "MIT"
metadata:
  version: 1.0.0
...
- `[Req: formal ??README Â§3]` ??written by humans in a spec document. Authoritative.
...
**Critical: Record the import pattern.** How do existing tests import project modules? Every language has its own conventions (Python `sys.path` manipulation, Java/Scala package imports, TypeScript relative paths or aliases, Go package/module paths, Rust `use crate::` or `use myproject::`). You must use the exact same pattern in your functional tests ??getting this wrong means every test fails with import/resolution errors. See `references/functional_tests.md` Â§ "Import Pattern" for the full six-language matrix.
...
4. **Check library versions** ??Check the project's dependency manifest (`requirements.txt`, `build.sbt`, `package.json`, `pom.xml`/`build.gradle`, `go.mod`, `Cargo.toml`) to see what's actually available. Don't write tests that depend on library features that aren't installed. If a dependency might be missing, use the test framework's skip mechanism ??see `references/functional_tests.md` Â§ "Library version awareness" for framework-specific examples.
...
Minimum bar: at least 2?? defensive patterns per core source file. If you find fewer, you're skimming ??read function bodies, not just signatures.
...
- **Test count heuristic** = (testable spec sections) + (QUALITY.md scenarios) + (defensive patterns). For a medium project (5??5 source files), this typically yields 35??0 tests. Significantly fewer suggests missed requirements or shallow exploration. Significantly more is fine if every test is meaningful ??don't pad to hit a number.
...
**Calibrate unit counts to the project.** Read `chunk_size` or equivalent config. Use enough units to span at least 2 chunks and enough to verify distribution checks. Typically 10??0 for integration testing.
...
|------|-------------|------------|------------|
| QUALITY.md | Quality constitution | 10 scenarios | ?ˆâ??ˆâ??ˆâ??‘â? High ??grounded in code, but scenarios are inferred, not from real incidents |
| Functional tests | Automated tests | 47 passing | ?ˆâ??ˆâ??ˆâ??ˆâ? High ??all tests pass, 35% cross-variant |
| RUN_CODE_REVIEW.md | Code review protocol | 8 focus areas | ?ˆâ??ˆâ??ˆâ??ˆâ? High ??derived from architecture |
| RUN_INTEGRATION_TESTS.md | Integration test protocol | 9 runs ? 3 providers | ?ˆâ??ˆâ??ˆâ??‘â? Medium ??quality gates need threshold tuning |
| RUN_SPEC_AUDIT.md | Council of Three audit | 10 scrutiny areas | ?ˆâ??ˆâ??ˆâ??ˆâ? High ??guardrails included |
| AGENTS.md | AI session bootstrap | Updated | ?ˆâ??ˆâ??ˆâ??ˆâ? High ??factual |
```
...
| `references/review_protocols.md` | Files 3?? (code review, integration) | Templates for both protocols |

...
description: "Use when visually reviewing or fixing website layout, responsive behavior, accessibility, and design consistency."
license: "MIT"
---
...
```
Detection targets:
?œâ??€ package.json     ??Framework and dependencies
?œâ??€ tsconfig.json    ??TypeScript usage
?œâ??€ tailwind.config  ??Tailwind CSS
?œâ??€ next.config      ??Next.js
?œâ??€ vite.config      ??Vite
?œâ??€ nuxt.config      ??Nuxt
?”â??€ src/ or app/     ??Source directory
```

...
> **同步注记（2026-03-27）：** 本译本仅反映 current-baseline 的一部分。英文 `README.md` 仍是 authoritative version。与 support、release boundary、review scope 相关的高风险段落已同步，其余内容可能仍保留较旧的解读。另外，`Install Python dependencies`、`Run the baseline checks`、`Cross-Platform Baseline`、`Current Material Sets` 这几个章节尚未在这里完整同步，相关内容应以英文 `README.md` 为准。

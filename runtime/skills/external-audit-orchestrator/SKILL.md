...
- classify reviewer gates as passed, failed, partial, or unavailable
...
5. Do not treat acknowledgement-only, request-only, timeout, lifecycle-only, or unavailable reviewer output as approval.
6. A reviewer gate can be `passed` only when substantive raw reviewer output exists and the normalized report records that gate as passed.
7. If a reviewer emits useful findings but the final normalized rerun fails, classify the gate as `partial`, use the findings, and report the normalization gap.
...
Read [references/reviewer-gate-failures.md](references/reviewer-gate-failures.md) before declaring a reviewer gate passed or when raw reviewer output is missing, incomplete, timed out, or unavailable.
...
4. Run the audit as read-only unless the user explicitly asked for auto-fix.
   - For fragile or newly configured reviewer lanes, run a tiny health check first.
   - If the first result is acknowledgement-only, timeout, shutdown, request-only, or unavailable, classify the gate and switch lanes instead of repeatedly nudging the same reviewer.
...
6. Decide next action.
   ...
   - `escalate-to-human`
   - `archive-only`
   - Use `accept` only for a `passed` reviewer gate.
...
- Treat TB2 request JSON, manifests, or other template-only artifacts as `failed` or `partial` gates until live reviewer output is captured.

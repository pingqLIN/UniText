...
```bash
python -m unittest tests.security.test_hardening tests.test_registry_inventory tests.test_bootstrap_verify_smoke tests.test_project_map_baseline
```

若環境使用 `py` 啟動 Python：

```powershell
py -3 -m unittest tests.security.test_hardening tests.test_registry_inventory tests.test_bootstrap_verify_smoke tests.test_project_map_baseline
```

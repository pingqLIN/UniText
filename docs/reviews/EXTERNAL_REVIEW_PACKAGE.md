...
> Authoritative machine-readable contract: [external-review-bundle.contract.json](external-review-bundle.contract.json)
...
實際 export 與後續自動化應以 `external-review-bundle.contract.json` 為唯一 machine-readable source，再由本文件補充人類可讀的範圍解釋。
...
目前 review package 的 canonical membership 由 `external-review-bundle.contract.json` 定義；本節只保留人類可讀的分類摘要。
...
`export-review-package.ps1` 會直接讀取 `external-review-bundle.contract.json`，因此若要調整 package 成員或 reading order，應先改 contract，再跑 export。
